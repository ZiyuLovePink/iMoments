...
-(UIButton*)addbackButtonWithTitle:(id)title{
    ...
    button.frame = CGRectMake(5, 0, normal.size.width-10, normal.size.height);
...
//        self.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:@"取消" style:UIBarButtonItemStylePlain target:self action:@selector(cancelButtonPressed:)];
        ...
        self.navigationItem.title = @"Cancel Passcode";//KKPasscodeLockLocalizedString(@"Enter Passcode", @"");
    }
...
- (void)incrementFailedAttemptsLabel
{
	...
	if (_failedAttemptsCount == 1) {
		_failedAttemptsLabel.text = @"Not correct!";//KKPasscodeLockLocalizedString(@"1 Failed Passcode Attempt", @"");
	} else {
		_failedAttemptsLabel.text = @"Not correct!";//[NSString stringWithFormat:KKPasscodeLockLocalizedString(@"%i Failed Passcode Attempts", @""), _failedAttemptsCount];
	}
		...
		if (_mode == KKPasscodeModeSet) {
			if ([textField isEqual:_setPasscodeTextField]) {
				...
				if (![_confirmPasscodeTextField.text isEqualToString:_setPasscodeTextField.text]) {
					...
					_passcodeConfirmationWarningLabel.text = @"Passcodes did not match. Try again.";//KKPasscodeLockLocalizedString(@"Passcodes did not match. Try again.", @"");
					[self moveToPreviousTableView];
				} else {
...
- (UIView*)headerViewForTextField:(UITextField*)textField
{
    ...
    if (_mode == KKPasscodeModeSet) {
        if ([textField isEqual:_enterPasscodeTextField]) {
            headerLabel.text = @"Enter your passcode";//KKPasscodeLockLocalizedString(@"Enter your passcode", @"");
        } else if ([textField isEqual:_setPasscodeTextField]) {
            headerLabel.text = @"Enter a passcode";//KKPasscodeLockLocalizedString(@"Enter a passcode", @"");
        } else if ([textField isEqual:_confirmPasscodeTextField]) {
            headerLabel.text = @"Re-enter your passcode";//KKPasscodeLockLocalizedString(@"Re-enter your passcode", @"");
        }
    } else if (_mode == KKPasscodeModeDisabled) {
        headerLabel.text = @"Enter your passcode";//KKPasscodeLockLocalizedString(@"Enter your passcode", @"");
    } else if (_mode == KKPasscodeModeChange) {
