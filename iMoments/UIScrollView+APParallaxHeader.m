...
-(APParallaxView *)getParallaxView
{
    if (!self.parallaxView) {
...
- (void)scrollViewDidScroll:(CGPoint)contentOffset {
    ...
    if (contentOffset.y<-280) {
        [self performSelector:@selector(runDelegate:) withObject:nil afterDelay:1.0];
    }
...
-(void)runDelegate:(id)sender
{
    [_appDelegate scrollViewScroll];
}
