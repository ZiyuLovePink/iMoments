...
#define VIEW_ORIGIN_X 185
#define VIEW_ORIGIN_Y 33
...
-(void)viewDetail{
    ...
    [self.view setFrame:CGRectMake(VIEW_ORIGIN_X, VIEW_ORIGIN_Y, (MARGIN_X+BTN_W_H)*ONE_ROW_ICON+MARGIN_X, BUTTOM_MARGIN + (BTN_W_H+MARGIN_Y)*x -160)];
...
-(void)endAnimation{
    ...
    if (_iconNumber == -1) {
        //No Select
        tmpDic = [NSDictionary dictionaryWithObject:@"emoticonDefault.png" forKey:@"icon"];
    }else{
        //Did Select
        tmpDic = [NSDictionary dictionaryWithObject:[_iconArray objectAtIndex:_iconNumber] forKey:@"icon"];
    }
    
    [_delegate oneEmoticonPressed:tmpDic];
