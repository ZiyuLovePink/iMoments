...
- (id)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        _sharers = [[NSMutableArray alloc] initWithObjects: [CFSharer pinterest], [CFSharer googleDrive], [CFSharer twitter], [CFSharer facebook], [CFSharer weibo],[CFSharer evernote], [CFSharer dropbox], [CFSharer mail], [CFSharer photoLibrary], nil];
...
- (void)setUpCircleLayers {
    ...
    _overlayLayer.frame = self.bounds;
    _overlayLayer.backgroundColor = [UIColor colorWithWhite:0.0 alpha:0.3].CGColor;
...
- (void)setUpSharingOptionsView {
    ...
    // Add table view.
    UITableView *tableView = [[UITableView alloc] initWithFrame:CGRectMake(0.0f, 45.0f, _sharingOptionsView.frame.size.width, _sharingOptionsView.frame.size.height-100)];
...
- (void)animateMoreOptionsOut {
    ...
    [_delegate shareCircleDisappear:nil];
}
